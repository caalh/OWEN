...
- **3D zoom on a full core is gradual again.** The old zoom stepped by a
  fraction of the distance to the *centre of the model*. On a pin cell that
  centre is a few centimetres away, so each scroll notch felt fine. On a
  BEAVRS core it is metres past the first pin, so one notch jumped from the
  vessel onto a single fuel pin — or inside the fuel, which went black.
  Scroll now zooms toward whatever is under the cursor, in the same small
  steps a pin cell always had. Drag to orbit and right-drag to pan are
  unchanged. If you do pass through a pin, the view no longer goes black.
- **A blank 3D view after a GPU hiccup now comes back.** If the webview
  loses its graphics context, OWEN throws the dead canvas away and builds a
  new one instead of staying black forever. Quality stays full until that
  actually happens.
- **Grid-spacer bands now render in MCNP, Serpent, and SCONE axial views,
  not just OpenMC.** The ~5.7 cm Inconel spacers at the eight BEAVRS
  elevations were only drawn on the OpenMC path. All four codes now show
  those gray bands.
